...
import { getScoreConfig } from "../../../../lib/helpers/score-config";
...
const GaugeChart = ({ percentage }: GaugeChartProps) => {
  ...
  return (
    <div 
      ref={cardRef}
      className={`gauge-card ${isVisible ? 'visible' : ''}`}
    >
      {/* Background glow */}
      <div 
        className="gauge-glow" 
        style={{ background: `radial-gradient(circle at 50% 80%, ${config.color}20 0%, transparent 70%)` }}
      />

      <div className="gauge-header">
        <div className="gauge-icon" style={{ background: `linear-gradient(135deg, ${config.gradient[0]} 0%, ${config.gradient[1]} 100%)` }}>
      ...
      <div className="gauge-chart-container">
        <svg className="gauge-svg" viewBox="0 0 200 140">
          ...
          <circle cx="100" cy="120" r="8" fill={config.color} className="needle-base" />
          ...
          {/* Value labels */}
          <text x="15" y="135" className="gauge-label-text" textAnchor="middle">0%</text>
          <text x="100" y="25" className="gauge-label-text" textAnchor="middle">50%</text>
          <text x="185" y="135" className="gauge-label-text" textAnchor="middle">100%</text>
        </svg>
      ...
      <div className="gauge-scale">
        <div className="scale-segment" style={{ '--segment-color': '#ef4444' } as React.CSSProperties}>
          <span className="segment-dot" />
          <span className="segment-label">0-40%</span>
        </div>
        <div className="scale-segment" style={{ '--segment-color': '#f59e0b' } as React.CSSProperties}>
          <span className="segment-dot" />
          <span className="segment-label">40-60%</span>
        </div>
        <div className="scale-segment" style={{ '--segment-color': '#6366f1' } as React.CSSProperties}>
          <span className="segment-dot" />
          <span className="segment-label">60-80%</span>
        </div>
        <div className="scale-segment" style={{ '--segment-color': '#10b981' } as React.CSSProperties}>
          <span className="segment-dot" />
